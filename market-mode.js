...
export const MARKET_PRESETS = {
  bullish: {
    ...
    belowOORWaitMinutes: 10,   // Token dump di bawah range → keluar sangat cepat
    // Screening
    maxVolatility: 8,          // Izinkan volatility lebih tinggi (pasar aktif)
    minVolume: 1000,           // Volume minimum lebih tinggi
    maxEntry5mPricePct: 30,    // Skip jika sudah pump +30% (terlambat masuk)
  },
  ...
  bearish: {
    ...
    belowOORWaitMinutes: 8,    // Token dump di bawah range → keluar cepat sekali
    // Screening
    maxVolatility: 5,          // Hindari pool terlalu volatile
    minVolume: 1500,           // Butuh volume lebih tinggi untuk konfirmasi activity
    maxEntry5mPricePct: 15,    // Skip jika sudah pump (lebih ketat di bearish)
    minEntry5mPricePct: -20,   // Skip jika sudah dump banyak (jangan tangkap pisau jatuh)
  },
  ...
  sideways: {
    ...
    belowOORWaitMinutes: 15,   // Standard below-OOR wait
    // Screening
    maxVolatility: 6,          // Volatility sedang
    minVolume: 500,            // Volume normal
    maxEntry5mPricePct: 20,    // Skip jika sudah pump
  },
  ...
  volatile: {
    ...
    belowOORWaitMinutes: 20,   // Volatile → bisa recovery, tunggu sedikit lebih lama
    // Screening
    maxVolatility: 10,         // Izinkan pool sangat volatile
    minVolume: 3000,           // Volume tinggi = activity nyata pada volatile market
    maxEntry5mPricePct: 50,    // Lebih toleran pada pump di volatile market
  },
  ...
  conservative: {
    ...
    belowOORWaitMinutes: 8,    // Token dump → keluar sangat cepat
    ...
    maxEntry5mPricePct: 10,    // Hanya masuk ke pool yang harganya belum banyak bergerak
    minEntry5mPricePct: -10,   // Hindari token yang sudah drop
  },
};
...
export function applyPresetToConfig(preset, config) {
  ...
  if (preset.belowOORWaitMinutes    != null) config.management.belowOORWaitMinutes    = preset.belowOORWaitMinutes;
  ...
  if (preset.maxEntry5mPricePct    !== undefined) config.screening.maxEntry5mPricePct = preset.maxEntry5mPricePct;
  if (preset.minEntry5mPricePct    !== undefined) config.screening.minEntry5mPricePct = preset.minEntry5mPricePct;
}
